...
def train_model(
    ...
    with torch.no_grad():
        ...
        for i, ( inputs, weight ) in enumerate(testloader):
            ...
            recon_loss = Reconstruction_Loss(inputs,outputs.float(),weight)
...
for n_latent in [1,2,3,4]:
    model = model_VAE(n_latent)
    model = model.to(device)
    try:
        modelfile = 'model_hpc/model weighted chi latent{} a{} beta{}.pth'.format(n_latent,a,beta)
        model.load_state_dict(torch.load(modelfile))
    except:
        modelfile = 'model_hpc/model weighted chi latent{} a{} beta{}.pth'.format(n_latent,1,1)
        model.load_state_dict(torch.load(modelfile))
    history = train_model(
        model,
        modelfile = 'model_hpc/model weighted chi latent{} a{} beta{}.pth'.format(n_latent,a,beta),
